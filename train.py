...
for (patter_sentece, tag) in xy:
    bag = preprocess_dataset.bag_of_words(patter_sentece, all_words)
    X_train.append(bag)
...
class ChatDataset(Dataset):
    ...
    def __len__(self):
        ...
        Retorna:
           - nos retornara el tamaño de los datos de ejemplo.
        """
...
for epoch in range(num_epoch):
    for (words, labels) in train_loader:
        ...
        optimizer.step()
    if (epoch + 1) % 100 == 0:
        print(f"epoch: {epoch+1}/{num_epoch}, loss = {loss.item():.7f}")
